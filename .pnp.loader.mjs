...
function fromPortablePathWin32(p) {
  ...
  if ((portablePathMatch = p.match(PORTABLE_PATH_REGEXP)))
    ...
    (uncPortablePathMatch = p.match(UNC_PORTABLE_PATH_REGEXP))
  )
...
function toPortablePathWin32(p) {
  ...
  if ((windowsPathMatch = p.match(WINDOWS_PATH_REGEXP)))
    ...
    (uncWindowsPathMatch = p.match(UNC_WINDOWS_PATH_REGEXP))
  )
    ...
    if (!indexStat) {
      if (atomicBehavior === 0 /* Lock */) {
        await destinationFs.lockPromise(indexPath, async () => {
          ...
          await destinationFs.writeFilePromise(
            indexPath,
            content
          );
        });
...
class ProxiedFS extends FakeFS {
  ...
  async lutimesPromise(p, atime, mtime) {
    return this.baseFs.lutimesPromise(
      this.mapToBase(p),
      atime,
      mtime
    );
  ...
  lutimesSync(p, atime, mtime) {
    return this.baseFs.lutimesSync(
      this.mapToBase(p),
      atime,
      mtime
    );
...
class NodeFS extends BasePortableFakeFS {
  ...
  truncateSync(p, len) {
    return this.realFs.truncateSync(
      npath.fromPortablePath(p),
      len
    );
  }
...
class VirtualFS extends ProxiedFS {
  ...
  mapToBase(p) {
    ...
    const resolvedP = VirtualFS.resolveVirtual(
      this.baseFs.resolve(p)
    );
    return (
      ppath.relative(resolvedRoot, resolvedP) || PortablePath.dot
    );
  }
  mapFromBase(p) {
    return p;
...
function getPackageConfig(path, specifier, base, readFileSyncFn) {
  ...
  let { imports, main, name, type } = filterOwnProperties(
    packageJSON,
    ["imports", "main", "name", "type"]
  );
  const exports = ObjectPrototypeHasOwnProperty(
    packageJSON,
    "exports"
  )
    ? packageJSON.exports
...
function resolvePackageTargetString(
  ...
  if (
    RegExpPrototypeExec(invalidSegmentRegEx, subpath) !== null
  ) {
...
function patternKeyCompare(a, b) {
  ...
  const baseLenA =
    aPatternIndex === -1 ? a.length : aPatternIndex + 1;
  const baseLenB =
    bPatternIndex === -1 ? b.length : bPatternIndex + 1;
...
async function resolve$1(
  ...
  const issuer =
    parentURL && tryParseURL(parentURL)?.protocol === `file:`
      ...
      : process.cwd();
  const pnpapi =
    findPnpApi(issuer) ?? (url ? findPnpApi(specifier) : null);
  if (!pnpapi)
    return nextResolve(originalSpecifier, context, nextResolve);
  if (specifier.startsWith(`#`))
    return resolvePrivateRequest(
      specifier,
      issuer,
      context,
      nextResolve
  ...
  if (!result)
    throw new Error(
      `Resolving '${specifier}' from '${issuer}' failed`
    );
...
if (!HAS_LAZY_LOADED_TRANSLATORS) {
  ...
  const originalReadFile =
    binding.readFileUtf8 || binding.readFileSync;
  if (originalReadFile) {
