...
introduce you to all the basic concepts involved. If you want to learn about the
syntax of the QML language first, then take a look at the
[QML Language Reference](/docs/guide/qml-language).
...
But if the base `quickshell` folder contains a `shell.qml` file, subfolders will
not be considered.
...
- [PanelWindow](/docs/types/quickshell/panelwindow), for bars and widgets
...
the clock component in the bar need to be dealt with, as well as the process
and timer that make up the actual clock.
