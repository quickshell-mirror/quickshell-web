...
Rounded windows are simply transparent, square window with an opaque/translucent,
rounded rectangle inside of them.
...
@@Quickshell.PanelWindow {
  ...
  @@QtQuick.Rectangle {
    ...
    anchors.fill: parent // matches the size of the window
    radius: 5
  }
}
```
...
or any list that can be scrolled, you want a @@QtQuick.ListView instead.
...
or using Loaders if what you need is more complex.

The @@QtQuick.Item (and its subclasses) has the @@QtQuick.Item.visible property.
This can be easily used to change the visibility of an Item conditionally:

```qml
@@QtQuick.Item { // or any of its subclasses
  visible: false
}
```

If you want to display either one of at least two components conditionally, you can use Loaders:
...
Use @@QtQuick.Loader if the component being loaded inherits from @@QtQuick.Item. Otherwise,
you should use @@Quickshell.LazyLoader instead.
...
use @@Quickshell.QsWindow.surfaceFormat, and set `opaque` to false.
