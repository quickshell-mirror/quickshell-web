...
Since Quickshell 0.1 has been released, you can now choose whether to install by tracking
the master branch, or install by latest release.
...
The package is available as `quickshell.packages.<system>.default`, which you can then add to
...
> [!WARNING]
> When using the AUR package, Quickshell may break whenever Qt is updated.
...
- Qmlls does not work well when a file is not correctly structured.
  ...
  correctly and such.
- Qmlls cannot handle Quickshell's @@Quickshell.Singleton, which means you won't see
  completions, and usages of Singleton members may show a warning. We're still investigating
