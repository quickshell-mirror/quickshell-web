...
You should have a way to track specific revisions with your distribution to avoid
breakage if a user updates Quickshell before you can update your configuration.
...
### Configuration Paths
The ideal configuration path depends on how you distribute your config.
This is because Quickshell can load configs from a number of different paths.
...
Any of the named configurations present in the directory can then be used using the Quickshell
command by specifying `--config` or `-c`, like so: `qs -c <name>`.

#### As a package
Some configurations are distributed as distro packages. These packages should use the
