...
> [!TIP]
> Quickshell has a builtin component, which is the @@Quickshell.Widgets.WrapperItem,
...
the child's actual size. But the **`childrenRect` breaks this pattern.**

The property `childrenRect` encompasses the geometry of all child items, meaning their *actual* geometry,
not their *implicit* geometry. This results in the container item's size having an indirect
dependency on itself, which is known as a *binding loop*.
...
To solve the boilerplate problem often leading users to use `childrenRect`, Quickshell comes with
